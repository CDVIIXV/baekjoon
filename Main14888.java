...
{
	static final int plus = 0, minus = 1, multi = 2, divi = 3, operatorCount = 4, minAnswer = -1000000000, maxAnswer = 1000000000;
	
	static int[] numbers;
	static int min, max;
	
	public static void combi(int[] operator, int answer, int sign)
	{
		if(sign > 0)
		{	
			for(int i=0; i<operatorCount; ++i)
				...
				{
					int canswer = answer;
					int[] coperator = operator.clone();
					coperator[i]--;
					if(i == plus)
						canswer += numbers[numbers.length - sign];
					else if(i == minus)
						canswer -= numbers[numbers.length - sign];
					else if(i == multi)
						canswer *= numbers[numbers.length - sign];
					...
					combi(coperator, canswer, sign-1);
				}
		}
		else
		{
	...
	public static void main(String[] args)
	{
		...
		int[] operator = new int[operatorCount];
		for(int i=0; i<operatorCount; ++i)
			operator[i] = scan.nextInt();
		min = maxAnswer + 1;
		max = minAnswer - 1;
		combi(operator, numbers[0], numbers.length-1);
